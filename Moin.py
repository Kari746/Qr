...
BOT_TOKEN = "7638678019:AAHyEXajrkI93aDHO9x8VDUkEYC_cGmETQk"

# Admin password for /pass command
ADMIN_PASSWORD = "8163493962"

# Payment details
PAYMENT_AMOUNT = 500  # 500 INR for 7 days access
...
def execute_binary(ip: str, port: int, time: int):
    try:
        command = ["./Moin", ip, str(port), str(time)]
...
async def bgmi(update: Update, context: CallbackContext):
    ...
    if user_id not in user_access:
        await update.message.reply_text("⚠️ You don't have access. Please pay ₹500 for 7 days access using /pay. CONTACT @Topten_Kari")
        return
...
async def pay(update: Update, context: CallbackContext):
    ...
    qr.add_data(f"Pay ₹{PAYMENT_AMOUNT} for 7 days access.")
    qr.make(fit=True)
...
async def handle_transaction_id(update: Update, context: CallbackContext):
    ...
    if transaction_id.strip():
        ...
        await update.message.reply_text("✅ Payment verified! You now have 7 days access.")
    else:
...
async def trial(update: Update, context: CallbackContext):
    ...
    await context.bot.send_message(chat_id="8163493962", text=admin_message)
